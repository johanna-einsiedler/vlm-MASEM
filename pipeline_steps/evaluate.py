...
def _resolve_dataset_name(raw_name: str) -> str:
    ...
    Known short aliases (e.g. 'tune' -> 'tuning') are mapped via DATASET_MAP.
    Any other name (e.g. 'tuning_together_1step', 'tuning_1step') passes through as-is,
    allowing derived dataset tags from extract.py to be used directly.
    """
    return DATASET_MAP.get(raw_name, raw_name)
...
def _load_extraction_file(extraction_dir: Path, study_name: str, is_metadata: bool = False) -> Optional[Dict[str, Any]]:
    ...
    - New envelope format: {"filename": ..., "entries": [...], ...}
    - Legacy formats: {"study": ..., "pages": [...]} or {"records": [...]}
    """
    ...
    try:
        ...
        # New envelope format: {"entries": [...]}
        if "entries" in extraction_raw:
            return {
                "study": study_name,
                "samples": extraction_raw["entries"],
            }

        # Legacy metadata format: {"records": [...]}
        if is_metadata and "records" in extraction_raw:
            records = extraction_raw.get("records") or []
            return {"study": study_name, "samples": records}

        # Legacy factors/correlations format: {"study": ..., "pages": [{"result": "..."}]}
        ...
        # Legacy: {"result": "..."}
        if "result" in extraction_raw:
            return _extract_json_blob(extraction_raw["result"])

        # Assume already extracted data
    ...
    if num_samples > 1:
        ...
        for sample_idx, gt_idx in enumerate(best_permutation):
            ...
            for key, value in metadata.items():
                if key in ["factor_loadings", "factor_correlations", "evidence", "notes", "sample_id", "source_sample_id"]:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=None,
        help="PDF file name (e.g., wise2000.pdf). Omit to evaluate all studies in --dataset.",
    )
    parser.add_argument(
        "--dataset",
        help="Dataset name (e.g., tuning_together_1step). Required when pdf_name is omitted.",
    )
    parser.add_argument(
        "--excel",
    ...
    parser.add_argument(
        "--plots",
        action="store_true",
        help="Generate accuracy figures after evaluation (batch mode only).",
    )
    ...
    if args.pdf_name:
        ...
        if not args.dataset:
            parser.error("--dataset is required when pdf_name is omitted.")
        dataset = _resolve_dataset_name(args.dataset)
        # Collect study names from whichever extraction folder exists
        for subdir in ("extraction_factors", "extraction_correlations", "extraction_metadata"):
            folder = Path("data") / subdir / dataset
            if folder.exists():
                studies = sorted(p.name for p in folder.glob("*.json"))
                break
        else:
            print(f"No extraction files found for dataset '{dataset}'")
            return
        print(f"Evaluating {len(studies)} study/studies in '{dataset}'...")
        for study_file in studies:
            print(f"\n--- {study_file} ---")
            run(study_file, dataset=args.dataset, excel_path=args.excel, force=args.force)
        if args.plots:
            import importlib.util
            spec = importlib.util.spec_from_file_location(
                "accuracy_plots", Path(__file__).parent / "accuracy_plots.py"
            )
            plots_mod = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(plots_mod)
            print(f"\n--- Generating figures for '{dataset}' ---")
            plots_mod.run(dataset=dataset, force=args.force)
