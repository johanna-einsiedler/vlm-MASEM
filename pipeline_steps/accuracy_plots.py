...
def run(
    ...
    # Create metadata plot (single bar — non-zero comparison not meaningful for metadata)
    if not force and output_files["metadata"].exists():
        ...
        plt.figure(figsize=(max(10, len(studies) * 0.5), 6))
        plt.bar(range(len(studies)), meta_all_vals, color="steelblue")
        plt.xticks(range(len(studies)), studies, rotation=90)
        plt.ylim(0, 1)
        plt.ylabel("Accuracy")
        plt.title("Metadata Accuracy by Study")
        plt.tight_layout()
        plt.savefig(output_files["metadata"], dpi=200)
        plt.close()
